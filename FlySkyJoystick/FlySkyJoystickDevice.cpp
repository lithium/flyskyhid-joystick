...
OSDefineMetaClassAndStructors(FlySkyJoystickDevice, super)

bool FlySkyJoystickDevice::parseHidDescriptor(
...
FlySkyJoystickDevice *FlySkyJoystickDevice::withHidDescriptor(
                                                      const void *hidDescriptorData,
                                                      size_t hidDescriptorDataSize,
                                                      OSString *productString)
{
    FlySkyJoystickDevice *result = new FlySkyJoystickDevice();
...
bool FlySkyJoystickDevice::init(
                            const void *hidDescriptorData,
                            size_t hidDescriptorDataSize,
                            OSString *productString,
...
IOReturn FlySkyJoystickDevice::newReportDescriptor(IOMemoryDescriptor **descriptor) const
{
...
OSString *FlySkyJoystickDevice::newTransportString() const
{
    return OSString::withCString("Virtual");
}

OSNumber *FlySkyJoystickDevice::newPrimaryUsageNumber() const
{
    return OSNumber::withNumber(m_Capabilities.usage, 32);
}

OSNumber *FlySkyJoystickDevice::newPrimaryUsagePageNumber() const
{
    return OSNumber::withNumber(m_Capabilities.usagePage, 32);
}

OSNumber *FlySkyJoystickDevice::newLocationIDNumber() const
{
    return OSNumber::withNumber(m_LocationID, 32);
}

bool FlySkyJoystickDevice::updateState(const void *hidData, size_t hidDataSize)
{
...
void FlySkyJoystickDevice::free()
{
